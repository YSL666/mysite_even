...
def get_blog_list_common_date(request,blogs_all_list):
    paginator=Paginator(blogs_all_list,settings.EACH_PAGE_BLOGS_NUM) #每2篇进行分页,用了settings
    page_num=request.GET.get('page',1) #获取url的页面参数（GET请求）
    page_of_blogs=paginator.get_page(page_num)
    current_page_num=page_of_blogs.number #获取当前页码
    #获取当前页码前后各5页码
    page_range=[i for i in range(current_page_num-2,current_page_num+3) if i in paginator.page_range]
